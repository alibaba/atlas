...
public class ResourceReleaser {
    ...
    public static boolean releaseResource(File apkFile, File revisionDir) throws IOException {
        ...
        String extractTag = "assets";
        for (Enumeration entries = bundleFile.entries(); entries.hasMoreElements(); ) {
            ZipEntry zipEntry = (ZipEntry)entries.nextElement();
            String entryName = zipEntry.getName();
            if((entryName.startsWith(extractTag) || entryName.equals("AndroidManifest.xml")) && !entryName.equalsIgnoreCase("../")) {
                int numAttempts = 0;
                ...
                while(numAttempts<3 && !isExtractionSuccessful) {
                    ...
                    try {
                        ...
                        if(entryName.equals("AndroidManifest.xml")){
                            targetPath = String.format("%s%s%s%s%s", revisionDir, File.separator, "newAssets", File.separator,
                                    entryName.substring(entryName.lastIndexOf(File.separator) + 1, entryName.length()));
                        }else {
                            targetPath = String.format("%s%s%s%s%s", revisionDir, File.separator, "newAssets/assets", File.separator,
                                    entryName.substring(entryName.lastIndexOf(File.separator) + 1, entryName.length()));
                        }
                ...
                if(!zipEntry.isDirectory() && entryName.startsWith(extractTag)) {
