...
public class BundleInstaller implements Callable{
    ...
    private static List<Pair<String,InstallListener>>    sIdleInstallBundles = new ArrayList<Pair<String,InstallListener>>();
    private static List<Pair<String,InstallListener>>    sDelayInstallBundles = new ArrayList<Pair<String,InstallListener>>();
    ...
    public synchronized static void createIdleInstallerIfNeed(){
        if(sIdleHandler==null){
            sIdleHandler = new MessageQueue.IdleHandler() {
                @Override
                public boolean queueIdle() {
                    if (sIdleInstallBundles.size() == 0 && sDelayInstallBundles.size() == 0) {
                        sIdleHandler = null;
                        return false;
                    }
                    Pair<String,InstallListener> bundleStruct = null;
                    if (sDelayInstallBundles.size() > 0) {
                        bundleStruct = sDelayInstallBundles.remove(0);
                    } else if (sIdleInstallBundles.size() > 0) {
                        bundleStruct = sIdleInstallBundles.remove(0);
                    }
                    if(bundleStruct!=null) {
                        String bundleName = bundleStruct.first;
                        InstallListener listener = bundleStruct.second;
    ...
    public static void startIdleInstall(final String location,InstallListener listener){
        createIdleInstallerIfNeed();
        if(location!=null){
            BundleImpl impl = (BundleImpl)Atlas.getInstance().getBundle(location);
            if(impl==null || !impl.checkValidate()){
                sIdleInstallBundles.add(new Pair<String, InstallListener>(location,listener));
            }
        }

    }

    public static void startDelayInstall(final String location,InstallListener listener)
    {
        createIdleInstallerIfNeed();
        if(location!=null){
            BundleImpl impl = (BundleImpl)Atlas.getInstance().getBundle(location);
            if(impl==null || !impl.checkValidate()){
                sDelayInstallBundles.add(new Pair<String, InstallListener>(location,listener));
            }
        }
    }
    ...
    void release(){
        mLocation = null;
        mBundleSourceInputStream = null;
        mBundleSourceFile = null;
        mListener = null;
        mTmpBundleSourceFile = null;
        mTmpBundleSourceInputStream = null;
        mTransitive = false;
    }
...
//     * @param location
//     */
//    public void installTransitivelyOnUiThreadSync(String[] location){
//        if (location!=null){
//            mTransitive = true;
//            mLocation = location;
//            try {
...
//    }
    ...
    /************************************************************************************/
    /////////////////////////////////////安装外部bundle////////////////////////////////////
    public void installAsync(final String[] location, final InputStream[] input, InstallListener listener){
        if(Thread.currentThread().getId() != Looper.getMainLooper().getThread().getId()){
            throw new RuntimeException("method executeOnUIThread is only prepared for UI Thread");
        }
        release();
        mListener = listener;
        mBundleSourceInputStream = input;
        mLocation = location;
        try {
    ...
    public void installAsync(final String[] location, final File[] input, InstallListener listener){
        if(Thread.currentThread().getId() != Looper.getMainLooper().getThread().getId()){
            throw new RuntimeException("method executeOnUIThread is only prepared for UI Thread");
        }
        release();
        mListener = listener;
        mBundleSourceFile = input;
        mLocation = location;
        try {
            checkSrc(location,input);
            installBundleInternal(false);
    ...
    public void installSync(final String[] location, final InputStream[] input){
        try {
            release();
            mBundleSourceInputStream = input;
            mLocation = location;
    ...
    public void installSync(final String[] location, final File[] input){
        try {
            release();
            mBundleSourceFile = input;
            mLocation = location;
...
//    /**
//     * bundle from apk only
//     * @param location
//     * @return
//     */
//    public Bundle installSync(final String[] location){
//        try {
//            mLocation = location;
//            return installBundleInternal(true);
//        }catch(Throwable e){
//            e.printStackTrace();
//            BundleInstallerFetcher.recycle(this);
//            return null;
...
//    }
                    ...
                    try {
                        ...
                        if (mListener != null) {
                            new Handler(Looper.getMainLooper()).post(new Runnable() {
                                @Override
                                public void run() {
                                    Log.e("BundleInstaller", "notify finish "+ Arrays.toString(mLocation));
                                    if(mListener!=null) {
                                        mListener.onFinished();
                                    }
    ...
    public Bundle getInstalledBundle(String bundleName, Map<String, Bundle> bundles) {
        ...
        if (bundle == null) {
            try {
                ...
                if (bundle == null) {
                    bundle = Framework.restoreFromExistedBundle(bundleName, bundles);
                }
            } finally {
                // if (lockSuccess) {
                try {
    ...
    public synchronized Void call() throws Exception {
        Map<String, Bundle> bundles = new ConcurrentHashMap<String, Bundle>();
        try {
            Bundle bundle = null;
            if (!mTransitive) {
                for (int x = 0; x < mLocation.length; x++) {
                    if (FileUtils.getUsableSpace(Environment.getDataDirectory()) >= 5) {
                        if (mBundleSourceInputStream != null && mBundleSourceInputStream.length > x
                            ...
                            if ((bundle = getInstalledBundle(mLocation[x], bundles)) == null) {
                                if ((BaselineInfoManager.instance().isDexPatched(mLocation[x]) || BaselineInfoManager
                                    .instance().isUpdated(mLocation[x]))) {
                                    continue;
                                }
                                bundle = Framework.installNewBundle(mLocation[x],
                                        mBundleSourceInputStream[x],
                            ...
                            if ((bundle = getInstalledBundle(mLocation[x], bundles)) == null
                                    && AtlasBundleInfoManager.instance()
                                .isInternalBundle(mLocation[x])) {
                                if ((BaselineInfoManager.instance().isDexPatched(mLocation[x]) || BaselineInfoManager
                                    .instance().isUpdated(mLocation[x]))) {
                                    continue;
                                }
                                bundle = installBundleFromApk(mLocation[x], bundles);
                                if (bundle != null) {
                                    ((BundleImpl)bundle).optDexFile();
                                }
                            }
                ...
                for (int x = 0; x < mLocation.length; x++) {
                    ...
                    for (String bundleName : bundlesForInstall) {
                        if ((bundle = getInstalledBundle(bundleName, bundles)) == null) {
                            ...
                            if (FileUtils.getUsableSpace(Environment.getDataDirectory()) >= 5) {
                                //has enough space
                                if (AtlasBundleInfoManager.instance().isInternalBundle
                                        (bundleName)) {
                                    bundle = installBundleFromApk(bundleName, bundles);
                                    if (bundle != null) {
                                        ((BundleImpl)bundle).optDexFile();
                                    }
                                }
            ...
            Framework.bundles.putAll(bundles);
        }
    }
    ...
    private Bundle installBundleFromApk(String bundleName, Map<String, Bundle> bundles)
            throws Exception {
        Bundle bundle = null;
        findBundleSource(bundleName);
        if(mTmpBundleSourceFile!=null){
            bundle = Framework.installNewBundle(bundleName, mTmpBundleSourceFile, bundles);
