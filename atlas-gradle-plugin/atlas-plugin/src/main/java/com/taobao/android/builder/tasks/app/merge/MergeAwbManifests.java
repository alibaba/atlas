...
public class MergeAwbManifests extends ManifestProcessorTask {
    ...
    public String getPackageOverride() {
        //不能返回和manbundle一样的package,不然aapt在merge resource 的时候会报资源冲突
    ...
    List<File> getLibraryManifests() {
        List<ManifestProvider> libs = getLibraries();
        if (libs == null || libs.isEmpty()) {
            return Collections.emptyList();
        }

        List<File> files = Lists.newArrayListWithCapacity(libs.size() * 2);
        for (ManifestProvider mdi : libs) {
    ...
    public void setLibraries(List<ManifestProvider> libraries) {
