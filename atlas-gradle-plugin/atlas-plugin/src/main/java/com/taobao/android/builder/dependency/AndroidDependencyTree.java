 ...
 */
public class AndroidDependencyTree {

    private List<ResolvedDependencyInfo> mResolvedDependencies = Lists.newArrayList();

    public List<ResolvedDependencyInfo> getmResolvedDependencies() {
        return mResolvedDependencies;
    }

    public void setmResolvedDependencies(List<ResolvedDependencyInfo> mResolvedDependencies) {
    ...
    public void setAarBundles(List<AarBundle> aarBundles) {
        this.aarBundles = aarBundles;
    }
    ...
    public List<ApLibrary> getApLibraries() {
        return apLibraries;
    }

    public void setApLibraries(List<ApLibrary> apLibraries) {
    ...
    public List<ApkLibrary> getApkLibraries() {
        return apkLibraries;
    }

    public void setApkLibraries(List<ApkLibrary> apkLibraries) {
        this.apkLibraries = apkLibraries;
    }

    public AndroidDependencyTree(List<ResolvedDependencyInfo> mResolvedDependencies) {
        this.mResolvedDependencies = mResolvedDependencies;
    }

    /**
     * 获取主bundle的所有manifest依赖
     *
     * @return
     */
    ...
    private void addChildDependency(List<String> deps, ResolvedDependencyInfo dependencyInfo) {
        for (ResolvedDependencyInfo child : dependencyInfo.getChildren()) {
            String childValue = getDependencyString(child);
            deps.add(childValue);
     ...
     * @return
     */
    private String getDependencyString(ResolvedDependencyInfo dep) {
        StringBuilder sb = new StringBuilder();
        sb.append(dep.getGroup()).append(":");
    ...
    public Set<SoLibrary> getALLSoLibDependencies() {

        Set<SoLibrary> dependencies = new HashSet<SoLibrary>();
        dependencies.addAll(getSoLibraries());

        List<AarBundle> aarBundles = new ArrayList<AarBundle>();
        aarBundles.addAll(getAarBundles());
        aarBundles.addAll(getAwbBundles());

        addSoLib(aarBundles, dependencies);
        return dependencies;
    }

    private void addSoLib(List<AarBundle> aarBundles, Set<SoLibrary> dependencies) {
        for (AarBundle aarBundle : aarBundles) {
            if (null != aarBundle.getSoLibraries() && aarBundle.getSoLibraries().size() > 0) {
    ...
    public Set<AarBundle> getALLAarDependencies() {
        ...
        Set<AarBundle> dependencies = new HashSet<AarBundle>();

        List<AarBundle> aarBundles = new ArrayList<AarBundle>();
        aarBundles.addAll(getAarBundles());
        aarBundles.addAll(getAwbBundles());

        addAarList(aarBundles, dependencies);

        return dependencies;
    }

    private void addAarList(List<AarBundle> aarBundles, Set<AarBundle> dependencies) {

        for (AarBundle aarBundle : aarBundles) {

            if (aarBundle.getClass().equals(AarBundle.class)) {
