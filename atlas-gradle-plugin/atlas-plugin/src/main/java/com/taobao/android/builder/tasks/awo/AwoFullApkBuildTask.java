...
import com.taobao.android.builder.tools.zipalign.ZipAlignUtils;
 ...
 * Created by shenghua.nish on 2016-07-18 4:11 this afternoon.
 */
public class AwoFullApkBuildTask extends DefaultAndroidTask {
    ...
    public void setBaseApkFile(File baseApkFile) {
    ...
    public void setAwoFile(File awoFile) {
        this.awoFile = awoFile;
    }
    ...
    public void setOutApkFile(File outApkFile) {
    ...
    public static class ConfigAction extends MtlBaseTaskAction<AwoFullApkBuildTask> {
        ...
        public String getName() {
            return libVariantContext.getScope().getTaskName("fullApkBuild");
        }

        @Override
        public Class<AwoFullApkBuildTask> getType() {
            return AwoFullApkBuildTask.class;
        }

        @Override
        public void execute(AwoFullApkBuildTask task) {
            task.setAndroidBuilder(libVariantContext.getScope().getGlobalScope().getAndroidBuilder());
            task.setVariantName(libVariantContext.getVariantName());
            task.setZipAlign(true);

            if (!atlasExtension.getBundleConfig().isAwoApkBuild()) {
                task.setEnabled(false);
                return;
            }

            ConventionMappingHelper.map(task, "signingConfig", new Callable<DefaultSigningConfig>() {

                @Override
                public DefaultSigningConfig call() {
                    return libVariantContext.getSigningConfig();
                }
            });
            ConventionMappingHelper.map(task, "baseApkFile", new Callable<File>() {
            ...
            ConventionMappingHelper.map(task, "destPath", new Callable<String>() {

                @Override
                public String call() {
                    return "lib/armeabi/" + libVariantContext.getPackageFile().getName();
                }
            });
            ...
            ConventionMappingHelper.map(task, "outApkFile", new Callable<File>() {

                @Override
                public File call() {
                    VariantScope scope = libVariantContext.getScope();
                    return scope.getGlobalScope().getProject().file(scope.getGlobalScope().getApkLocation() + "/"
                            + scope.getGlobalScope().getProjectBaseName() + "-"
                            + libVariantContext.getVariantConfiguration().getDirName()
                            + ".apk");
                }
            });
        }
