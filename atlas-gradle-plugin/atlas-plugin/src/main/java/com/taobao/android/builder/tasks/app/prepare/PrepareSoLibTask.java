 ...
 */
public class PrepareSoLibTask extends BaseTask {
    ...
    void generate() {
        ...
        for (File jniFolder : getJniFolders()) {
            if (jniFolder.exists() && jniFolder.isDirectory()) {
                NativeSoUtils.copyLocalNativeLibraries(jniFolder, getMainBundleOutputFolder(),
        ...
        for (SoLibrary mainSoLib : getMainDexSoLibraries()) {
            ...
            if (explodeFolder.exists() && explodeFolder.isDirectory()) {
                NativeSoUtils.copyLocalNativeLibraries(explodeFolder, getMainBundleOutputFolder(),
                        getSupportAbis(), getRemoveSoFiles(), getILogger());
            }
        }
        ...
        for (AwbBundle awbLib : getAwbLibs()) {
            ...
            if (awbJniFolder.exists() && awbJniFolder.isDirectory()) {
                NativeSoUtils.copyLocalNativeLibraries(awbJniFolder, awbOutputFolder,
                        getSupportAbis(), getRemoveSoFiles(), getILogger());
            }
            //为了兼容之前老的aar，awb格式
            ...
            for (AndroidLibrary dep : deps) {
                ...
                if (depLibsFolder.exists() && depLibsFolder.isDirectory()) {
                    NativeSoUtils.copyLocalNativeLibraries(depLibsFolder, awbOutputFolder,
                            getSupportAbis(), getRemoveSoFiles(), getILogger());
                }
            }
            ...
            if (null != solibs) {
                for (SoLibrary solib : solibs) {
                    ...
                    if (explodeFolder.exists() && explodeFolder.isDirectory()) {
                        NativeSoUtils.copyLocalNativeLibraries(explodeFolder, awbOutputFolder,
                                getSupportAbis(), getRemoveSoFiles(), getILogger());
                    }
                }
            }
        ...
        //判断是否有重复的so文件
        // 进行重复文件的查询
        Map<String, Multimap<String, File>> soMaps = NativeSoUtils.getAbiSoFiles(getSupportAbis(), getRemoveSoFiles(),
                scanDirs);
...
//        }
    ...
    public static class ConfigAction extends MtlBaseTaskAction<PrepareSoLibTask> {

        AndroidDependencyTree dependencyTree;
        ...
        public String getName() {
            return scope.getTaskName("copy", "SoLib");
        }
         ...
         */
        @Override
        public Class<PrepareSoLibTask> getType() {
            return PrepareSoLibTask.class;
        }
         ...
         */
        @Override
        public void execute(PrepareSoLibTask copySoLibTask) {
            ...
            ConventionMappingHelper.map(copySoLibTask, "jniFolders", new Callable<Set<File>>() {
                ...
                public Set call() throws Exception {
                    ...
                    for (AarBundle aarBundle : dependencyTree.getAarBundles()) {
            ...
            ConventionMappingHelper.map(copySoLibTask, "mainDexSoLibraries", new Callable<List>() {
                @Override
                public List call() throws Exception {
                    return dependencyTree.getSoLibraries();
                }
            });
            ...
            ConventionMappingHelper.map(copySoLibTask, "removeSoFiles", new Callable<Set>() {
                @Override
                public Set call() throws Exception {
                    return appVariantContext.getAtlasExtension().getTBuildConfig().getRemoveSoFiles();
                }
            });
