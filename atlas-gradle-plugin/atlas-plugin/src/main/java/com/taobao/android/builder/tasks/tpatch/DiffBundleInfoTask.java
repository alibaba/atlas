...
public class DiffBundleInfoTask extends BaseTask {
    ...
    private Set<ArtifactBundleInfo> getArtifactBundleInfo(DependencyDiff dependencyDiff, File mainfestFile, File apDir) throws IOException, DocumentException {
        ...
        AndroidDependencyTree androidDependencyTree = AtlasBuildContext.androidDependencyTrees.get(appVariantOutputContext.getVariantContext().
    ...
    private String getBundleName(LibraryDependency libraryDependency) {
        return libraryDependency.getResolvedCoordinates().getGroupId() + ":" + libraryDependency.getResolvedCoordinates().getArtifactId();
    }
            ...
            ConventionMappingHelper.map(diffBundleInfoTask, "dependencyDiff", new Callable<DependencyDiff>() {
                ...
                public DependencyDiff call() throws Exception {
                    ...
                    if (null != appVariantContext.apContext.getApExploredFolder() && appVariantContext.apContext.getApExploredFolder().exists()) {
                        DependencyJson dependencyJson = AtlasBuildContext.androidDependencyTrees.get(scope.getVariantOutputData().variantData.getName()).getDependencyJson();
                        File baseDependencyFile = new File(appVariantContext.apContext.getApExploredFolder(), "dependencies.txt");
