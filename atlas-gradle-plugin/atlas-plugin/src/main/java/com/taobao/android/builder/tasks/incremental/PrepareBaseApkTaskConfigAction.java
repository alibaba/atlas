...
public class PrepareBaseApkTaskConfigAction extends MtlBaseTaskAction<Sync> {
    ...
    public void execute(@NonNull Sync prepareBaseApkTask) {
        final FileTree baseApkFiles = variantContext.getProject().zipTree(variantContext.apContext.getBaseApk());
        prepareBaseApkTask.from(baseApkFiles, new Closure(PrepareBaseApkTaskConfigAction.class) {
            public Object doCall(CopySpec cs) {
                int dexFilesCount = 0;
                Set<File> dexFolders = scope.getVariantScope().getTransformManager().getPipelineOutput(StreamFilter.DEX)
                    .keySet();
                ...
                Set<File> baseDexFileSet = baseApkFiles.matching(new PatternSet().include("classes*.dex")).getFiles();
