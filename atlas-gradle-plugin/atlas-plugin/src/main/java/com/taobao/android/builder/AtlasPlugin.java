...
public class AtlasPlugin implements Plugin<Project> {
    ...
    public AtlasBuilder tAndroidBuilder;
    ...
    public void apply(final Project project) {
        ...
        createExtendsion();
        //增加一个components.android for aar/awb
        createLibCompenents();
    ...
    private void checkPluginSetup() {
        if (!PluginTypeUtils.isAndroidProject(project)) {
        ...
        if (!PLUGIN_ACCEPTABLE_VERSIONS.matcher(androidVersion).matches()) {
            String errorMessage = String.format("Android Gradle plugin version %s is required. Current version is %s. ", PLUGIN_MIN_VERSIONS, androidVersion);
    ...
    @NotNull
    protected void prepareBuildTools(Project project,ExtraModelInfo extraModelInfo, boolean customAapt) {
        AtlasBuildContext.androidBuilder = new AtlasBuilder(project.equals(project.getRootProject()) ? project.getName() : project.getPath(), creator, new GradleProcessExecutor(project),
                new GradleJavaProcessExecutor(project), extraModelInfo, getLogger(), isVerbose(), customAapt);
    }
